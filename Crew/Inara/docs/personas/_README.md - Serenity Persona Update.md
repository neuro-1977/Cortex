...
- All the charts, guides, and references are stored right here in our local NeDB database (`serenity_help.db`), ready even when we're off the grid.
...
1.  **Run Everything:**
    ...
    - This will fire up all our core services (NeDB, Blockly, Discord bot) in separate terminals, with health checks and status messages to keep you informed.

2. **Manual Start (for a closer look, should you need it):**
   - NeDB: `npm run start-server` (from root)
   - Blockly: `cd blockly-workspace && npm run start -- --port 8081`
   - Discord Bot: `cd Bots && node discord-bot.js`
...
- `Bots/` — Our trusty Discord bot, keeping an ear to the ground (check `.env` for its comms token).
...
- `public/`, `src/`, `tools/`, etc. — Supporting gear and assets, making everything run smooth.
