...
using Serenity.Cortex.Core.LLM;
        ...
        public static async Task InitializeCrewAsync()
        {
            ...
            foreach (var member in CrewPersonas)
            {
                ...
                try 
                {
                    ...
                    // Log to console/Discord log
                    DiscordService.Instance.QueueLog($"[Crew] {name}: {status}");
                    
                    // Log to file
                    string logEntry = $"[{DateTime.Now:yyyy-MM-dd HH:mm:ss}] {name}: {status}\n";
                    await System.IO.File.AppendAllTextAsync(logPath, logEntry);

                    // Post to Discord (staggered)
                    await Task.Delay(2000);
                    
                    await DiscordService.Instance.SendWebhookMessageAsync(name, status);
                }
                catch (Exception ex)
                {
                    DiscordService.Instance.QueueLog($"[Error] Failed to wake {member.Key}: {ex.Message}");
                }
            }
        }
    }
